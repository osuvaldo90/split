# Milestone v1.2: Test Foundation
...
Comprehensive test coverage to prevent regressions in authorization, calculations, and real-time sync. Added 140 tests (131 unit + 9 E2E) covering all critical paths.
...
**Key Decisions:**
- edge-runtime environment for Vitest to match Convex production runtime
- webServer reuseExistingServer for faster E2E iteration
- environmentMatchGlobs for convex tests (node env for convex-test, edge-runtime for calculations)
- Pure function tests in edge-runtime (no convex-test needed for calculations.ts)
- browser.newContext() for multi-user E2E isolation

**Issues Resolved:**
- Fixed convex-test environment configuration (glob is not a function in edge-runtime)
- Fixed strict mode violations in E2E selectors (.first() for duplicate matches)

**Issues Deferred:**
- SYNC-01, SYNC-02, SYNC-03 (real-time sync tests) — Multi-context WebSocket testing complexity

**Technical Debt:**
- convex/test.setup.ts exports unused convexTest helper (tests import directly from convex-test)
